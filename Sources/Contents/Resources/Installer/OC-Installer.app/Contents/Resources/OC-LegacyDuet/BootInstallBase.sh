...
This option (1) is only for use on a macOS installation USB Drive\nCancel for Exit" buttons {"Cancel","OK"} default button 2 with icon POSIX file "/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns" ')
USB=$(echo $response | cut -d ':' -f2)


# Exit if Canceled
if [ ! "$USB" ] ; then
   osascript -e 'display notification "Program closing" with title "USB OC-Legacy Duet" subtitle "User Cancel"'
  echo "USB OC-Legacy Duet -> User Cancel"
  exit 0
fi
...
Press the OK button to accept\nExit for Quit" buttons {"Exit", "OK"} default button 2 with icon POSIX file "/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns"  ')
...
echo "-------------------------------------"
Sleep 2
mkdir -p "$(diskutil info  disk"${N}"s1 |  sed -n 's/.*Mount Point: *//p')/NVRAM"
/usr/libexec/PlistBuddy -c "Merge './nvram.used' " "$(diskutil info  disk"${N}"s1 |  sed -n 's/.*Mount Point: *//p')/NVRAM/nvram.plist"
echo "Create nvram.plist -> /Volumes/EFI/NVRAM "
cp -v nvram.used "$(diskutil info  disk"${N}"s1 |  sed -n 's/.*Mount Point: *//p')/NVRAM"
fi