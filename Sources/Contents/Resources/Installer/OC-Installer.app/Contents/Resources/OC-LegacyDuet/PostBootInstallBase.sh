...
This option (2) is only for use on a macOS System Disk\nCancel for Exit" buttons {"Cancel","OK"} default button 2 with icon POSIX file "/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns" ')
POST=$(echo $response | cut -d ':' -f2)


# Exit if Canceled
if [ ! "$POST" ] ; then
   osascript -e 'display notification "Program closing" with title "Post OC-Legacy Duet" subtitle "User Cancel"'
  echo "Post OC-Legacy Duet -> User Cancel"
  exit 0
fi
...
response=$(osascript -e 'tell app "System Events" to display dialog "
Create the nvram.plist file !
Press the OK button to accept\n" buttons {"OK"} default button 1 with icon POSIX file "/System/Library/CoreServices/Finder.app/Contents/Resources/Finder.icns" giving up after 5 ')
LGHOOK_ROOT_DIR=$(echo $response | cut -d ':' -f2)


# Exit if Canceled
if [ "$LGHOOK_ROOT_DIR" == "Exit" ] ; then
   osascript -e 'display notification "Program closing" with title "NVRAM No" subtitle "User Exit"'
  echo "NVRAM install -> User Exit"
fi




if [ "$LGHOOK_ROOT_DIR" == "OK" ] ; then
echo "Proceed. "
fi

LGHOOK_ROOT_DIR="/Library/PrivilegedHelperTools/org.acidanthera.nvramhook.nvramdump.helper"

if [ -e "$LGHOOK_ROOT_DIR" ]; then
        echo "-------------------------------------"
	echo "NVRAM install NO ❌ LogoutHook is already installed, It will be Uninstall"
        sudo rm -rf "/Library/PrivilegedHelperTools/org.acidanthera.nvramhook.nvramdump.helper"
        sudo rm -rf "/Library/PrivilegedHelperTools/org.acidanthera.nvramhook.helper"
        sudo rm -rf "/Library/LaunchDaemons/org.acidanthera.nvramhook.daemon.plist"
        echo "-------------------------------------"
        echo "Uninstall OK ✅  "
        echo "OC-Legacy Duet will Exit in 10 sec Reboot then use Option(2)"
        for (( i = 0; i < 10; i++ ))
        do
            printf "."
            sleep 1
        ...
        echo "LogoutHook is not installed, it will be installed. "
        echo "-------------------------------------"
        Sleep 2
        rm -rf "$(diskutil info  disk"${N}"s1 |  sed -n 's/.*Mount Point: *//p')/NVRAM/nvram.used"
        rm -rf "$(diskutil info  disk"${N}"s1 |  sed -n 's/.*Mount Point: *//p')/NVRAM/nvram.plist"
        ./LogoutHook/Launchd.command install 
fi